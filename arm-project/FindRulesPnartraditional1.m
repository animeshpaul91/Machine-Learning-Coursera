function [Rules, FreqPItemsets, FreqNItemsets] = FindRulesPnartraditional1(transactions, minSup, dms, mc , nRules, sortFlag, fname)
%
                            ...
                            if ct > nRules
                                ...
                                corr = S - (sa*sb); % Corr(A,B)
                                   if corr > 0
                                     ...
                                     if Conf >= mc && S1 >= dms % Produce Rules of type ~A=>~B
                                         Rules{1}{ct} = -A;
                            ...
                            if c > nRules
                                ...
                                corr = S - (sa*sb); % Corr(A,B)
                                   if corr < 0
                                     ...
                                     if Conf1 >= mc && S2 >= dms % Produce Rules of type A=>~B
                                         %Store the rules that have minSup and minConf
                                         Rules{3}{c} = A;
                                     ...
                                     if Conf2 >= mc && S3 >= dms % Produce Rules of type ~A=>B
                                         Rules{3}{c} = -A;
                                         Rules{4}{c} = B; % All Negative itemsets are prefixed with a '-' sign to identify them.
                                         NRuleConf(c) = Conf2;
                                         NRuleSup(c) = S3;
