function [Rules, FreqItemsets] = findRulesapriori(transactions, minSup, minConf, nRules, sortFlag, labels, fname)
%
...
%
% Author: Animesh Paul 02/05/2015 
...
switch sortFlag
    case 1 % Sort by Support level
        [~, ind] = sort(RuleSup,'descend');
    case 2 % Sort by Confidence level
        [~, ind] = sort(RuleConf,'descend');
end

% V Holds support and ind holds index/serial number
% ind;items 
RuleConf = RuleConf(ind);
RuleSup = RuleSup(ind);
% to arrange Ruleconf and RuleSup cell array in the descending order of Support


% to arrange Rules{i,1} cell array in the descending order of Support
for i = 1:2
    temp = Rules{i};
    temp = temp(ind);
    Rules{i} = temp;
