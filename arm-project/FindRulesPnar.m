function [Rules, FreqPItemsets, FreqNItemsets] = FindRulesPnar(transactions, minSup, dms, P_mc, N_mc, mincorr, nRules, sortFlag, fname)
%
...
%          P_mc:                   scalar value that represents the minimum
%                                  threshold for confidence of each rule 
%                                  of type A=>B and ~A=>~B. 
%          N_mc:                   scalar value that represents the minimum
%                                  threshold for confidence of each rule 
%                                  of type ~A=>B and A=>~B.
...
FreqPItemsets = cell(maxSize,1); %Frequent Positive Itemsets
FreqNItemsets = cell(maxSize,1); %Frequent Negative Itemsets
PRuleConf = zeros(nRules,1);
PRuleSup = zeros(nRules,1);
NRuleConf = zeros(nRules,1);
NRuleSup = zeros(nRules,1);
PRuleCorr= zeros(nRules,1);
NRuleCorr= zeros(nRules,1);
ct = 1; % For Rules A=>B and ~A=>~B
c = 1;  % For Rules A=>~B and ~A=>B
co = 0; % Additional Counter to display the percentage of Positive Correlation
co1 = 0;
                    ...
                    for depth = 1:steps-1
                        ...
                        for r = 1:size(R,1)
                            if ct > nRules
                                ...
                                if abs(corr) >= mincorr % Check for interestingness
                                   if corr > 0
                                       if corr >= 0.20
                                         co = co + 1;
                                       end  
                                     % Generate potential rules and check for P_mc. Produce Rules of the Form A=>B and ~A=>~B
                                     % Calculate the confidence of the rule
                                     % Produce Rules of type A=>B
                                     Ctemp = S/sa; % Computing Conf(A=>B) 
                                     if Ctemp >= P_mc % Produce Rules of type A=>B
                                     ...
                                     if Conf >= P_mc && S1 >= dms % Produce Rules of type ~A=>~B
                                         Rules{1}{ct} = -A;
                            ...
                            if c > nRules
                                ...
                                if abs(corr) >= mincorr % Check for interestingness
                                   if corr < 0
                                     ...
                                     % Generate potential rules and check for N_mc. Produce Rules of the Form A=>~B and ~A=>B
                                     % Calculate the confidence of the rule
                                     % Produce Rules of type A=>~B
                                     S2 = (sa - S); % Computing Sup(A=>~B)
                              
                                     Conf1 = S2/sa; % Computing Conf(A=>~B)
                                     if Conf1 >= N_mc && S2 >= dms % Produce Rules of type A=>~B
                                         %Store the rules that have minSup and minConf
                                         Rules{3}{c} = A;
                                     ...
                                     if Conf2 >= N_mc && S3 >= dms % Produce Rules of type ~A=>B
                                         Rules{3}{c} = -A;
                                         Rules{4}{c} = B; % All Negative itemsets are prefixed with a '-' sign to identify them.
                                         NRuleConf(c) = Conf2;
                                         NRuleSup(c) = S3;
...
switch sortFlag
    ...
    case 3 % Sort by Correlation Level
        [~, ind] = sort(PRuleCorr,'descend');  % ind stores index of PRules in decreasing correlation
        [~, ind1] = sort(NRuleCorr,'descend'); % ind stores index of NRules in decreasing correlation
end
...
PRuleCorr = PRuleCorr(ind);
NRuleCorr = NRuleCorr(ind1);
...
fprintf(fid,'\n\n\nWriting Rules A=>B and ~A=>~B into File\n\n');
fprintf(fid, '%s   (%s, %s, %s) \n\n', 'Rule', 'Support', 'Confidence', 'Correlation Factor');
...
for l = 1:size(Rules{3},1)
    ...
    s10 = num2str(NRuleCorr(l));
     fprintf(fid, '%s => %s  (%s%%, %s%%, %s)\n', s5, s6, s7, s8, s10);
end
t = (co/(ct + c))*100;
t1 = (co1/(ct + c))*100;
% fprintf(fid, '\nThe Percentage of Rules having Correlation Factor >= 0.20 is  ' a '%%\n');
disp(['The Percentage of Rules having Correlation factor >= 0.20 is  ']);
t
disp('%');
disp(['The Percentage of Rules having Correlation factor <= -0.25 is  ']);
t1
disp('%');
fclose(fid);
end